...
public:
    ...
    void init(int __attribute__((unused)) sda = 0, int __attribute__((unused)) scl = 0)
    {
        ...
        setCursorXY(0, 0);
        if (_BUFF)
            setWindow(0, 0, _maxX, _maxRow); // для буфера включаем всю область
    }
    ...
    virtual size_t write(uint8_t data)
    {
        ...
        if (data > 127)
        {
            ...
            else if (_lastChar == 208 && data == 134)
                data = 73; // замена укр. I на англ
            else if (_lastChar == 209 && data == 150)
                data = 105; // замена укр. i на англ
            else if (_lastChar == 208 && data == 135)
                data = 193; // замена укр. Ї
            else if (_lastChar == 209 && data == 151)
                data = 194; // замена укр. ї
            else if (_lastChar == 208 && data == 132)
                data = 195; // замена укр. Є
            else if (_lastChar == 209 && data == 148)
                data = 196; // замена укр. є
            _lastChar = thisData;
        }
        if (data == 0)
            return 1;
            ...
            else
            {
                ...
                {                                      // 6 стобиков буквы
                    ...
                    { // если масштаб 1
                        ...
                        { // внутри дисплея
                            if (_shift == 0)
                            {                                 // если вывод без сдвига на строку
                    ...
                    {                         // масштаб 2, 3 или 4 - растягиваем шрифт
    ...
    void dot(int x, int y, byte fill = 1)
    {
        ...
        else
        {
            ...
            else
            {
                x -= _bufX;
                y -= _bufY;
                if (x < 0 || x > _bufsizeX || y < 0 || y > (_bufsizeY << 3))
                    return;
        ...
        else
        {
            ...
            if (_buf_flag)
            {
                ...
                for (byte x = x0; x <= x1; x++)
                    fastLineV(x, y0, y1, fill == OLED_FILL ? 1 : 0);
                return;
            }
    ...
    void drawBitmap(int x, int y, const uint8_t *frame, int width, int height, uint8_t invert = 0, byte mode = 0)
    {
        ...
        { // в пикселях
            byte prevData = 0;
            if (_inRange(X, 0, _maxX))
            { // мы внутри дисплея по X
                for (int Y = y >> 3, countY = 0; Y < shiftY; Y++, countY++)
                {                                                                          // в строках (пикс/8)
